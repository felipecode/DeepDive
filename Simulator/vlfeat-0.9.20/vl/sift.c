...
VL_INLINE  float Mymax(float a,float b){
	return a>b?a:b;
}
VL_INLINE  float Mymin(float a,float b){
	return a<b?a:b;
}
/** ------------------------------------------------------------------
 ** @internal
...
vl_sift_detect (VlSiftFilt * f)
{
  ...
  int x, y, s, i, ii, jj ;
  vl_sift_pix *pt, v ,tempDog,tempNdog;
  VlSiftKeypoint *k ;

  /* clear current list */
  f-> nkeys = 0 ;

  /* compute difference of gaussian (ii-DoG) */
  pt = f-> dog ;
  for (s = s_min ; s <= s_max - 1 ; ++s) {
    ...
    while (src_a != end_a) {
    	tempDog= *src_b - *src_a;
    	tempNdog=tempDog / (*src_b + *src_a);
    	*pt++=Mymax(Mymax(tempNdog,0),Mymax(tempDog,0))+Mymin(Mymin(tempNdog,0),Mymin(tempDog,0));
      *src_b++;
      *src_a++ ; 
    }
  }
