...
def residual(tf, x,base_name,training=True):
  ...
  b_conv=deep_dive.bias_variable([64])

  conv = tf.nn.elu(deep_dive.conv2d(x_image, W_conv,strides=[1, 1, 1, 1], padding='SAME')+B_conv)
  print conv
  ...
  b_conv2=deep_dive.bias_variable([64])
  ...
  conv2 = deep_dive.conv2d(x_image, W_conv2,strides=[1, 1, 1, 1], padding='SAME')+B_conv2
  print conv2
