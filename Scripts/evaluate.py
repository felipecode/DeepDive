...
for i in im_names:
  ...
  original.shape = (1,)+ original.shape
  x = tf.placeholder("float",shape=(1,im.shape[0],im.shape[1])+config.input_size[-1:], name="input_image")
  ...
  feedDict.update({x: original})
  output = sess.run(h_conv3, feed_dict=feedDict)

  original.shape = original.shape[+1:]
  ...
  #print im.shape[0]
  result = result.resize((output.shape[1], original.shape[0]), Image.ANTIALIAS)
